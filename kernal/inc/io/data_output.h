...
// @return The state at the end of the operation or `-1` if a invaild channel is given
...
// @return `0` If config is suported, if the buffering state is not bit 0 is set, if the resolution is not suported bit 1 is set, bit 2 is invaild channel
int dac_channel_supports_config(int channel, int resolution, int buffered);

// Main logic for dac_output_start, exists since dac_output_start is a wrapper
// that saves some values before launching _dac_output_start_internal
// if _dac_output_start_internal is called dirrectly dac_output_end
// wont work
