...
{   
    ...
    if (header->number_of_entrys > 1)
    {
        ...
        memcpy(new_buffer, header->ptr, index * header->size_of_entry);                 // Copy before the entry
        memcpy(((uint8_t*)new_buffer) + (index * header->size_of_entry),                // New offset of the next entry
            ((uint8_t*)header->ptr) + ((index + 1) * header->size_of_entry),            // Offset of the next entry in src
