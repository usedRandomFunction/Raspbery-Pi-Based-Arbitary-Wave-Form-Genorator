#include "central_block_memory_allocator.h"

#include "common/basic_io.h"
#include "common/memory.h"

// TODO tbh this allocator is kinda bad, I should make a new one
// but I feel like it atm, so this is what we have
//
// anyway here is the extra stuff i need to add so it works
...
// Also did i forget to comment theses?
...
void initialize_central_block_memory_allocator(void* mem_start, size_t region_size, size_t block_size_as_power_of_two, central_block_memory_allocator_header* header)
{
    ...
    // |             | ←-n-→ |
    // |xxxxxxxxxxxx | xxxxx |
    // +=====kernal/src/lib/central_block_memory_allocator.c========+=======+
    ...
    printf("central_block_memory_allocator_%x: ", header);
    printf("Allocator Started\n");

    printf("central_block_memory_allocator_%x: ", header);
    printf("Controll Region uses: %d bytes\n", controll_region_size_blocks << block_size_as_power_of_two);

    printf("central_block_memory_allocator_%x: ", header);
    printf("Allocatable Space: %d Kib of %d Kib\n",
        (header->number_of_total_blocks << block_size_as_power_of_two) / 1024,
        region_size/ 1024);

    printf("central_block_memory_allocator_%x: ", header);
    printf("Total Blocks: %d\n", header->number_of_total_blocks);
...
void central_block_memory_allocator_free(void* ptr, central_block_memory_allocator_header* header)
{
    ...
    if (blockID >= header->number_of_total_blocks) // Is block does not exist, do free it
    {
        ...
        #ifdef CENTRAL_BLOCK_MEMORY_ALLOCATOR_EXIT_FAILED
        exit(-9999);
        #endif
