...
// @param delay_milliseconds When > 0 this is the delay between keypad frames, when = 0 it turns off the physical keypad when -1 just returns the current state and -2 just sets the value to defult.
...
// @param state 0 is dissabled 1 is enabled, -1 just returns the current state  -2 just sets the value to defult.
...
// @note is handler is NULL this will switch to the defult handler from the OS which closes the current app
